...
class AchievementSystem:
    def __init__(self):
        self.achievements = [
            Achievement(
                "First Steps", "Survive for 30 seconds", "🎯", 30, "time_survived"
            ),
            Achievement(
                "Neural Apprentice", "Reach 1000 training progress", "🧠", 1000, "score"
            ),
            ...
            Achievement(
                "Speed Demon", "Survive for 2 minutes", "⚡", 120, "time_survived"
            ),
            Achievement(
                "Neural Network", "Reach 5000 training progress", "🌐", 5000, "score"
            ),
            Achievement("Combo Legend", "Achieve a 20x combo", "🔥", 20, "combo"),
            Achievement(
                "Dodge Master", "Perform 50 perfect dodges", "💫", 50, "perfect_dodges"
            ),
            Achievement(
                "Training Veteran", "Survive for 5 minutes", "🏆", 300, "time_survived"
            ),
        ]
    ...
    def draw(self, screen):
        ...
        for achievement in self.active_notifications:
            ...
            pygame.draw.rect(
                screen,
                COLORS["WHITE"],
                (x, y, notification_width, notification_height),
                3,
            )
            ...
            title_text = font.render(
                f"{achievement.icon} {achievement.name}", True, COLORS["WHITE"]
            )
            desc_text = small_font.render(
                achievement.description, True, COLORS["WHITE"]
            )
