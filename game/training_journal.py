...
class TrainingSession:
    ...
    def end_session(
        self, final_score, final_distance, obstacles, powerups, abilities, cause
    ):
...
class Achievement:
    ...
    def __init__(
        self, id, name, description, condition_func, icon="🏆", rarity="common"
    ):
        self.id = id
        self.name = name
...
class TrainingJournal:
    ...
    def __init__(self):
        ...
        self.journal_rect = pygame.Rect(
            100, 50, SCREEN_WIDTH - 200, SCREEN_HEIGHT - 100
        )
        ...
        if not self.achievements:  # Only initialize if empty
            self.achievements = [
                ...
                Achievement(
                    "novice_runner",
                    "Novice Runner",
                    "Score 1,000 points in a single run",
                    lambda d: d["best_score"] >= 1000,
                    "🏃",
                    "common",
                ),
                Achievement(
                ...
                Achievement(
                    "expert_neuron",
                    "Expert Neuron",
                    "Score 10,000 points in a single run",
                    lambda d: d["best_score"] >= 10000,
                    "🧠",
                    "rare",
                ),
                Achievement(
                    "ai_master",
                    "AI Master",
                    "Score 25,000 points in a single run",
                    lambda d: d["best_score"] >= 25000,
                    "👑",
                    "legendary",
                ),
                ...
                Achievement(
                    "marathon_runner",
                    "Marathon Runner",
                    "Travel 10,000 units in total",
                    lambda d: d["total_distance"] >= 10000,
                    "🏃‍♂️",
                    "common",
                ),
                Achievement(
                    "ultra_marathoner",
                    "Ultra Marathoner",
                    "Travel 50,000 units in total",
                    lambda d: d["total_distance"] >= 50000,
                    "🏃‍♀️",
                    "uncommon",
                ),
                ...
                Achievement(
                    ...
                    lambda d: d["total_time_played"] >= 3600,
                    "⏰",
                    "common",
                ),
                Achievement(
                    "committed_researcher",
                    "Committed Researcher",
                    "Play for 5 hours total",
                    lambda d: d["total_time_played"] >= 18000,
                    "🔬",
                    "uncommon",
                ),
                ...
                Achievement(
                    "skin_collector",
                    "Skin Collector",
                    "Use all 4 different model skins",
                    lambda d: len(d.get("model_skin_usage", {})) >= 4,
                    "🎨",
                    "uncommon",
                ),
                Achievement(
                    "transformer_master",
                    "Transformer Master",
                    "Complete 10 runs with Transformer skin",
                    lambda d: d.get("model_skin_usage", {}).get("Transformer", 0) >= 10,
                    "🤖",
                    "uncommon",
                ),
                ...
                Achievement(
                    "obstacle_master",
                    "Obstacle Master",
                    "Dodge 1000 obstacles total",
                    lambda d: sum(
                        s.get("obstacles_dodged", 0)
                        for s in d.get("sessions_history", [])
                    )
                    >= 1000,
                    "🚧",
                    "rare",
                ...
                Achievement(
                    "powerup_enthusiast",
                    "Powerup Enthusiast",
                    "Collect 500 powerups total",
                    lambda d: sum(
                        s.get("powerups_collected", 0)
                        for s in d.get("sessions_history", [])
                    )
                    >= 500,
                    "⚡",
                ...
                Achievement(
                    "challenge_seeker",
                    "Challenge Seeker",
                    "Complete runs in all challenge modes",
                    lambda d: len(d.get("challenge_mode_attempts", {})) >= 4,
                    "⚔️",
                    "rare",
                ),
                ...
                Achievement(
                    "daily_trainer",
                    "Daily Trainer",
                    "Play on 7 different days",
                    lambda d: len(d.get("daily_stats", {})) >= 7,
                    "📅",
                    "uncommon",
                ),
                Achievement(
                    ...
                    "Play for 30 different days",
                    lambda d: len(d.get("daily_stats", {})) >= 30,
                    "🔥",
                    "legendary",
                ),
            ]
    ...
    def start_session(self, model_skin="CNN", challenge_mode=None):
        ...
        print(
            f"📊 Started training session with {model_skin}"
    ...
    def end_session(self, cause="collision"):
        ...
        if self.current_session.challenge_mode:
            ...
            self.challenge_mode_attempts[mode] = (
                self.challenge_mode_attempts.get(mode, 0) + 1
            )
        ...
        print(
            f"📊 Training session ended: {self.current_session.score} points, "
    ...
    def update(self):
        ...
        self.notification_queue = [
            n for n in self.notification_queue if current_time - n["timestamp"] < 3.0
        ]  # Keep for 3 seconds
    ...
    def draw(self, screen):
        ...
        journal_rect = pygame.Rect(
            current_x,
            self.journal_rect.y,
            self.journal_rect.width,
            self.journal_rect.height,
        )

        pygame.draw.rect(screen, COLORS["WHITE"], journal_rect, border_radius=15)
        pygame.draw.rect(
            screen, COLORS["NEURON_BLUE"], journal_rect, 4, border_radius=15
        )
        ...
        for i, (button, name) in enumerate(zip(self.page_buttons, page_names)):
            adjusted_button = pygame.Rect(
                button.x - slide_offset, button.y, button.width, button.height
            )

            if i == self.journal_page:
                pygame.draw.rect(
                    screen, COLORS["NEURON_BLUE"], adjusted_button, border_radius=8
                )
                text_color = COLORS["WHITE"]
            else:
                pygame.draw.rect(
                    screen, COLORS["LIGHT_GRAY"], adjusted_button, border_radius=8
                )
        ...
        content_rect = pygame.Rect(
            journal_rect.x + 20,
            journal_rect.y + 120,
            journal_rect.width - 40,
            journal_rect.height - 180,
        )
    ...
    def draw_stats_page(self, screen, rect):
        ...
        if rect.height > y_offset + 100:
            ...
            for day in recent_days:
                ...
                day_text = (
                    f"{day}: {stats['sessions']} sessions, {stats['score']:,} points"
                )
    ...
    def draw_history_page(self, screen, rect):
        ...
        for header, x_pos in zip(headers, header_x_positions):
            header_surface = pygame.font.Font(None, 22).render(
                header, True, COLORS["NEURON_BLUE"]
            )
        ...
        pygame.draw.line(
            ...
            COLORS["LIGHT_GRAY"],
            (rect.x, rect.y + 60),
            (rect.x + rect.width - 20, rect.y + 60),
            2,
        )
        ...
        for i, session in enumerate(reversed(recent_sessions)):
            ...
            # Format session data
            date_str = datetime.fromtimestamp(session["start_time"]).strftime(
                "%m/%d %H:%M"
            )
            score_str = f"{session['score']:,}"
            distance_str = f"{session['distance']:.0f}"
            duration_str = self.format_time(
                session.get("end_time", session["start_time"]) - session["start_time"]
            )
    ...
    def draw_achievements_page(self, screen, rect):
        ...
        for achievement in self.achievements:
            ...
            icon_surface = pygame.font.Font(None, 24).render(
                achievement.icon, True, icon_color
            )
            screen.blit(icon_surface, (rect.x, rect.y + y_offset))

            # Achievement name
            name_color = COLORS["DARK_BLUE"] if achievement.unlocked else COLORS["GRAY"]
            name_surface = pygame.font.Font(None, 22).render(
            ...
            desc_surface = font.render(
                achievement.description, True, COLORS["DARK_GRAY"]
            )
            ...
            pygame.draw.circle(
                screen,
                rarity_color,
                (rect.x + rect.width - 30, rect.y + y_offset + 15),
                8,
            )
    ...
    def draw_achievement_notification(self, screen, notification, index):
        ...
        notification_rect = pygame.Rect(
            SCREEN_WIDTH - 350 + x_offset, 100 + index * 80, 300, 60
        )

        # Background
        pygame.draw.rect(
            screen, COLORS["DARK_BLUE"], notification_rect, border_radius=10
        )
        pygame.draw.rect(
            screen, COLORS["YELLOW"], notification_rect, 3, border_radius=10
        )
        ...
        desc_text = desc_font.render(
            achievement.description[:40] + "...", True, COLORS["LIGHT_GRAY"]
        )
                    ...
                    for loaded_achievement in achievement_data:
                        for achievement in self.achievements:
                            if achievement.id == loaded_achievement["id"]:
                                achievement.unlocked = loaded_achievement.get(
                                    "unlocked", False
                                )
                                achievement.unlock_time = loaded_achievement.get(
                                    "unlock_time"
                                )
                                break

                print(
                    f"📊 Loaded training journal: {self.total_sessions} sessions, {self.best_score} best score"
                )
