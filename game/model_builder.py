...
class LayerProperty:
    ...
    def __init__(
        self, name, value, prop_type, min_val=None, max_val=None, options=None
    ):
        self.name = name
        self.value = value
...
class LayerComponent:
    ...
    def get_advanced_properties(self):
        ...
        if self.layer_type == "Dense":
            ...
            properties["activation"] = LayerProperty(
                "activation",
                "relu",
                "choice",
            ...
            properties["kernel_init"] = LayerProperty(
                "kernel_init",
                "glorot_uniform",
                "choice",
                options=[
            ...
            properties["padding"] = LayerProperty(
                "padding", "same", "choice", options=["same", "valid"]
            )
            properties["activation"] = LayerProperty(
                "activation",
                "relu",
            ...
            properties["return_sequences"] = LayerProperty(
                "return_sequences", True, "bool"
            )
            properties["return_state"] = LayerProperty("return_state", False, "bool")
            properties["dropout"] = LayerProperty("dropout", 0.0, "float", 0.0, 0.9)
            properties["recurrent_dropout"] = LayerProperty(
                "recurrent_dropout", 0.0, "float", 0.0, 0.9
            )
            properties["activation"] = LayerProperty(
                "activation",
                "tanh",
            ...
            properties["noise_shape"] = LayerProperty(
                "noise_shape", "None", "choice", options=["None", "spatial", "channel"]
            )
            ...
            properties["input_dim"] = LayerProperty(
                "input_dim", 10000, "int", 1, 100000
            )
            properties["output_dim"] = LayerProperty("output_dim", 128, "int", 1, 1024)
            properties["mask_zero"] = LayerProperty("mask_zero", False, "bool")
            properties["input_length"] = LayerProperty(
                "input_length", 100, "int", 1, 1000
            )
            ...
            properties["padding"] = LayerProperty(
                "padding", "valid", "choice", options=["valid", "same"]
            )
    ...
    def calculate_performance_impact(self):
        ...
        if self.layer_type == "Dense":
            neurons = self.properties.get(
                "neurons", LayerProperty("neurons", 128, "int")
            ).value
            ...
            filters = self.properties.get(
                "filters", LayerProperty("filters", 32, "int")
            ).value
            kernel = self.properties.get(
                "kernel_size", LayerProperty("kernel_size", 3, "int")
            ).value
            impact["memory_cost"] = (filters * kernel * kernel) / 288.0  # 32 * 3 * 3
            impact["speed_modifier"] = 1.0 + (filters / 128.0)

        elif self.layer_type == "LSTM":
            units = self.properties.get(
                "units", LayerProperty("units", 64, "int")
            ).value
            ...
            rate = self.properties.get(
                "rate", LayerProperty("rate", 0.2, "float")
            ).value
            ...
            key_dim = self.properties.get(
                "key_dim", LayerProperty("key_dim", 64, "int")
            ).value
...
class ModelBuilder:
    ...
    def is_in_palette_area(self, pos):
        ...
        palette_rect = pygame.Rect(
            self.palette_x, self.palette_y, 150, len(self.available_layers) * 50
        )
    ...
    def build_model(self):
        ...
        print(
            f"🧠 Built custom model: {model_depth} layers, {total_params:,} parameters"
        )
    ...
    def draw_layer_palette(self, screen):
        ...
        palette_title = pygame.font.Font(None, 32).render(
            "Layer Palette", True, COLORS["WHITE"]
        )
    ...
    def draw_controls(self, screen):
        ...
        # Close button
        pygame.draw.rect(
            screen, COLORS["DARK_GRAY"], self.close_button, border_radius=8
        )
