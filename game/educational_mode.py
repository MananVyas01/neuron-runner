...
class TutorialStep:
    ...
    def __init__(self, title, instruction, condition_type, condition_data, hint=None, highlight_area=None):
        self.title = title
        self.instruction = instruction
        self.condition_type = condition_type  # 'action', 'time', 'score', 'collision', 'powerup'
    ...
    def check_completion(self, game_state):
        ...
        if self.condition_type == 'action':
            ...
            return game_state.get('last_collision') == self.condition_data['obstacle_type']
...
class EducationalMode:
        ...
        tutorials.append(Tutorial(
            ...
            steps=[
                ...
                TutorialStep(
                    ...
                    "collision", {"obstacle_type": "OVERFITTING"},
    ...
    def start_next_tutorial(self):
        ...
        for tutorial in self.tutorials:
            if tutorial.id in self.unlocked_tutorials and tutorial.id not in self.completed_tutorials:
    ...
    def draw_tutorial_panel(self, screen):
        ...
        # Panel border
        border_color = COLORS['ACTIVATION'] if self.current_tutorial.step_completed else COLORS['DATASET']
        pygame.draw.rect(screen, border_color, panel_rect, 3)
        
        # Tutorial title
        title_font = pygame.font.Font(None, 36)
        title_text = title_font.render(self.current_tutorial.title, True, COLORS['WHITE'])
        ...
        instruction_lines = self.wrap_text(current_step.instruction, instruction_font, panel_rect.width - 40)
        
        for i, line in enumerate(instruction_lines):
            instruction_surface = instruction_font.render(line, True, COLORS['WHITE'])
            screen.blit(instruction_surface, (panel_rect.x + 20, panel_rect.y + 90 + i * 25))
        
        # Hint text
        if current_step.hint:
            hint_font = pygame.font.Font(None, 20)
            hint_text = hint_font.render(f"💡 {current_step.hint}", True, COLORS['YELLOW'])
            screen.blit(hint_text, (panel_rect.x + 20, panel_rect.y + panel_rect.height - 50))
        
        # Controls help
        controls_font = pygame.font.Font(None, 18)
        if self.current_tutorial.step_completed:
            controls_text = controls_font.render("Press N for next step | ESC to exit", True, COLORS['LIGHT_GRAY'])
        else:
            controls_text = controls_font.render("Complete the instruction above | S to skip | ESC to exit", True, COLORS['LIGHT_GRAY'])
        
        screen.blit(controls_text, (panel_rect.x + 20, panel_rect.y + panel_rect.height - 25))
    ...
    def draw_highlights(self, screen):
        ...
        if highlight['type'] == 'key':
            ...
            key_surface = key_font.render(key_text, True, (*COLORS['YELLOW'], pulse_alpha))
            
            key_rect = key_surface.get_rect(center=(SCREEN_WIDTH//2, SCREEN_HEIGHT//2 + 100))
    ...
    def draw_progress_indicator(self, screen):
        ...
        # Progress fill
        progress_ratio = (self.current_tutorial.current_step + 1) / len(self.current_tutorial.steps)
        ...
        step_rect = step_surface.get_rect(center=(progress_x + progress_width//2, progress_y + 25))
