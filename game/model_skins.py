...
class ModelSkin:
    def __init__(
        self, name, display_name, description, color_scheme, abilities, visual_style
    ):
        self.name = name
    ...
    def can_use_special(self, player):
        ...
        return (
            hasattr(player, "special_cooldown_timer")
            and player.special_cooldown_timer <= 0
        )
...
class ModelSelector:
    ...
    def draw_selection_screen(self, screen):
        ...
        title_text = self.font_large.render(
            "🧠 SELECT YOUR AI MODEL", True, COLORS["WHITE"]
        )
        ...
        cards_per_row = 2
        start_x = (
            SCREEN_WIDTH - (cards_per_row * card_width + (cards_per_row - 1) * 50)
        ) // 2
        start_y = 150
        ...
        for i, model_name in enumerate(self.models_list):
            ...
            if is_selected:
                ...
                highlight_rect = pygame.Rect(
                    card_x - 5 - pulse,
                    card_y - 5 - pulse,
                    card_width + 10 + pulse * 2,
                    card_height + 10 + pulse * 2,
                )
            ...
            self.draw_model_preview(
                screen, model, card_x + card_width // 2, card_y + 60
            )

            # Model name
            name_text = self.font_medium.render(
                model.display_name, True, COLORS["WHITE"]
            )
            name_rect = name_text.get_rect(
                center=(card_x + card_width // 2, card_y + 120)
            )
            ...
            for j, stat in enumerate(stats_text):
                ...
                stat_rect = stat_surface.get_rect(
                    center=(card_x + card_width // 2, stats_y + j * 20)
                )
        ...
        if self.selected_index < len(self.models_list):
            ...
            desc_panel_y = (
                start_y
                + ((len(self.models_list) + 1) // cards_per_row) * (card_height + 30)
                + 50
            )
            desc_width = 600
            desc_height = 120
            ...
            pygame.draw.rect(
                screen,
                selected_model.color_scheme["primary"],
                (desc_x, desc_panel_y, desc_width, desc_height),
                3,
            )
    ...
    def draw_model_preview(self, screen, model, x, y):
        ...
        if model.visual_style["shape"] == "geometric":
            ...
            for i in range(3):
                ...
                pygame.draw.circle(
                    screen,
                    model.color_scheme["glow"],
                    (int(glow_x), int(glow_y)),
                    radius,
                    2,
                )
            ...
            for i in range(4):
                ...
                pygame.draw.circle(
                    ring_surface, ring_color, (radius, radius), radius, 2
                )
            ...
            if int(self.preview_timer * 2) % 2:
                generator_color, discriminator_color = (
                    discriminator_color,
                    generator_color,
                )
