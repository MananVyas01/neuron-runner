...
class MLMetric:
    ...
    def __init__(self, name, initial_value, min_val=0, max_val=100, unit="", format_str="{:.2f}"):
        self.name = name
...
class MLStats:
    ...
    def __init__(self):
        ...
        self.training_metrics = {
            'learning_rate': MLMetric('Learning Rate', 0.001, 0.0001, 0.1, '', '{:.4f}'),
        ...
        self.health_metrics = {
            ...
            'convergence_rate': MLMetric('Convergence Rate', 1.0, 0, 3.0, 'x', '{:.2f}'),
        ...
        self.advanced_metrics = {
            'model_complexity': MLMetric('Model Complexity', 50.0, 0, 100, '', '{:.1f}'),
            'data_efficiency': MLMetric('Data Efficiency', 70.0, 0, 100, '%'),
            'computational_cost': MLMetric('Computational Cost', 1.0, 0.1, 10.0, 'x', '{:.2f}'),
            'memory_usage': MLMetric('Memory Usage', 512, 128, 8192, 'MB', '{:.0f}'),
            'inference_speed': MLMetric('Inference Speed', 100, 1, 1000, 'FPS', '{:.0f}'),
        }
    ...
    def setup_performance_baseline(self):
        ...
        for category in [self.metrics, self.training_metrics, self.health_metrics, self.advanced_metrics]:
    ...
    def update(self, game_stats, player_status):
        ...
        self.training_metrics['learning_rate'].update(0.001 + (game_stats.score / 100000.0))
        self.training_metrics['gradient_norm'].update(player_status.get('gradient_norm', 1.0))
        self.training_metrics['weight_norm'].update(5.0 + (game_stats.score / 500.0))
        self.training_metrics['batch_size'].update(32 + (game_stats.obstacles_dodged // 10))
        
        # Update health metrics
        self.health_metrics['gradient_health'].update(max(0, 100 - (game_stats.score // 100)))
        self.health_metrics['training_stability'].update(min(100, 50 + (game_stats.powerups_collected * 5)))
        self.health_metrics['convergence_rate'].update(min(3.0, 1.0 + (game_stats.score / 2000.0)))
        self.health_metrics['overfitting_risk'].update(min(100, game_stats.score / 100))
        self.health_metrics['generalization_gap'].update(max(0, 50 - (game_stats.obstacles_dodged / 10)))
    ...
    def get_performance_grade(self):
        ...
        overall_score = (self.accuracy + (100 - self.overfitting_risk) + self.gradient_health + self.training_stability) / 4
    ...
    def draw_ml_stats_panel(self, screen):
        ...
        if self.warning_flash > 0:
            ...
            border_color = (255, int(255 * (1 - flash_intensity)), int(255 * (1 - flash_intensity)))
        
        pygame.draw.rect(screen, border_color, (panel_x, panel_y, panel_width, panel_height), 3)
        
        # Title
        font_title = pygame.font.Font(None, 28)
        title_text = font_title.render("🧪 ML Performance Metrics", True, COLORS['WHITE'])
        ...
        # Accuracy with bar
        accuracy_text = font_metric.render(f"Accuracy: {self.accuracy:.1f}%", True, COLORS['WHITE'])
        screen.blit(accuracy_text, (panel_x + 10, y_offset))
        self.draw_metric_bar(screen, panel_x + 150, y_offset + 5, 150, 12, self.accuracy, 100, COLORS['ACTIVATION'])
        y_offset += 30
        ...
        self.draw_metric_bar(screen, panel_x + 150, y_offset + 5, 150, 12, loss_percent, 100, COLORS['GRADIENT'])
        y_offset += 30
        
        # Gradient Health
        grad_color = COLORS['ACTIVATION'] if self.gradient_health > 50 else COLORS['LOSS']
        grad_text = font_metric.render(f"Gradient Health: {self.gradient_health:.1f}%", True, COLORS['WHITE'])
        screen.blit(grad_text, (panel_x + 10, y_offset))
        self.draw_metric_bar(screen, panel_x + 180, y_offset + 5, 120, 12, self.gradient_health, 100, grad_color)
        y_offset += 30
        
        # Training Stability
        stability_color = COLORS['DATASET'] if self.training_stability > 70 else COLORS['OPTIMIZER']
        stability_text = font_metric.render(f"Training Stability: {self.training_stability:.1f}%", True, COLORS['WHITE'])
        screen.blit(stability_text, (panel_x + 10, y_offset))
        self.draw_metric_bar(screen, panel_x + 180, y_offset + 5, 120, 12, self.training_stability, 100, stability_color)
        y_offset += 30
        
        # Overfitting Risk (red is bad)
        risk_color = COLORS['LOSS'] if self.overfitting_risk > 50 else COLORS['ACTIVATION']
        risk_text = font_metric.render(f"Overfitting Risk: {self.overfitting_risk:.1f}%", True, COLORS['WHITE'])
        screen.blit(risk_text, (panel_x + 10, y_offset))
        self.draw_metric_bar(screen, panel_x + 180, y_offset + 5, 120, 12, self.overfitting_risk, 100, risk_color)
        y_offset += 30
        
        # Convergence Rate
        conv_text = font_metric.render(f"Learning Rate: {self.convergence_rate:.2f}x", True, COLORS['WHITE'])
    ...
    def draw_buffs_debuffs(self, screen, x, y):
        ...
        for buff in self.active_buffs:
            ...
            buff_text = font_small.render(f"✨ {buff['name']} ({remaining}s)", True, COLORS['ACTIVATION'])
        ...
        for debuff in self.active_debuffs:
            ...
            debuff_text = font_small.render(f"⚠️ {debuff['name']} ({remaining}s)", True, COLORS['LOSS'])
    ...
    def draw_mini_graphs(self, screen):
        ...
        # Accuracy graph
        self.draw_mini_graph(screen, graph_x, graph_y, graph_width, graph_height, 
                           self.accuracy_history, "Accuracy", COLORS['ACTIVATION'])
        
        # Loss graph
        self.draw_mini_graph(screen, graph_x, graph_y + 80, graph_width, graph_height, 
                           self.loss_history, "Loss", COLORS['GRADIENT'], invert=True)
        
        # Gradient health graph
        self.draw_mini_graph(screen, graph_x, graph_y + 160, graph_width, graph_height, 
                           self.gradient_history, "Gradient Health", COLORS['NEURON_BLUE'])
    
    def draw_mini_graph(self, screen, x, y, width, height, data, title, color, invert=False):
    ...
    def draw(self, screen):
        ...
        # Background
        bg_surface = pygame.Surface((stats_rect.width, stats_rect.height), pygame.SRCALPHA)
...
def update(self, game_stats, player_status):
        ...
        self.health_metrics['memory_usage'].update(min(95, 40 + (game_stats.powerups_collected * 5)))
        self.health_metrics['cpu_usage'].update(min(90, 30 + (game_stats.obstacles_dodged // 5)))
        ...
        self.advanced_metrics['rouge_score'].update(min(1.0, game_stats.powerups_collected / 50.0))
        self.advanced_metrics['f1_score'].update(min(1.0, game_stats.obstacles_dodged / 100.0))
