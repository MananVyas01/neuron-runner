...
class WorldTheme:
    ...
    def __init__(self, name, description, colors, effects, particle_style, music_intensity=1.0):
        self.name = name
    ...
    def draw_grid_overlay(self, screen, scroll_offset, pulse_timer):
        ...
        for x in range(-grid_size, SCREEN_WIDTH + grid_size, grid_size):
            ...
            pygame.draw.line(screen, grid_color, (adjusted_x, 0), (adjusted_x, SCREEN_HEIGHT))
    ...
    def draw_data_streams(self, screen, scroll_offset, pulse_timer):
        ...
        for i in range(stream_count):
            ...
            for j in range(5):
                x = ((scroll_offset * stream_speed + j * 150) % (SCREEN_WIDTH + 200)) - 100
                if 0 <= x <= SCREEN_WIDTH:
                    alpha = int(150 - j * 25)
        ...
        for col in range(column_count):
            ...
            for i in range(8):
                ...
                if 0 <= y <= SCREEN_HEIGHT:
                    ...
                    char_surface = pygame.Surface((column_width - 2, 12), pygame.SRCALPHA)
...
class WorldThemeManager:
    ...
    def complete_transition(self):
        ...
        print(f"✅ Theme transition complete: {self.themes[self.current_theme_index].name}")
    ...
    def get_background_color(self):
        ...
        blended_color = tuple(
            int(current_bg[i] * (1 - t) + next_bg[i] * t)
            for i in range(3)
        )
    ...
    def draw_background_effects(self, screen, scroll_offset):
        ...
        alpha_multiplier = 1.0 if not self.transitioning else (1.0 - self.transition_progress)
        if alpha_multiplier > 0:
            current_theme.apply_background_effects(screen, scroll_offset, self.pulse_timer)
        ...
        if self.transitioning and self.transition_progress > 0.3:
            ...
            if alpha_multiplier > 0:
                next_theme.apply_background_effects(screen, scroll_offset, self.pulse_timer)
    ...
    def draw_theme_notification(self, screen):
        ...
        pygame.draw.rect(screen, border_color, 
                        (notification_x, notification_y, notification_width, notification_height), 3)
        
        # Notification text
        font_large = pygame.font.Font(None, 32)
        font_small = pygame.font.Font(None, 24)
        
        title_text = font_large.render("🌍 ENTERING NEW DOMAIN", True, (*COLORS['WHITE'], alpha))
        title_rect = title_text.get_rect(center=(notification_x + notification_width//2, notification_y + 25))
        screen.blit(title_text, title_rect)
        
        theme_text = font_small.render(current_theme.name.upper(), True, (*current_theme.colors['primary'], alpha))
        theme_rect = theme_text.get_rect(center=(notification_x + notification_width//2, notification_y + 50))
        screen.blit(theme_text, theme_rect)
        
        desc_text = font_small.render(current_theme.description, True, (*COLORS['LIGHT_GRAY'], alpha))
        desc_rect = desc_text.get_rect(center=(notification_x + notification_width//2, notification_y + 70))
