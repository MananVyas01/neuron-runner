...
try:
    ...
    print(
        "⚠️ Replay system dependencies not available. Install imageio and Pillow for GIF recording."
    )
...
class ReplaySystem:
    ...
    def capture_frame(self, screen, stats, player_pos):
        ...
        if self.recording_quality == "low":
            ...
            # Moderate scaling
            scaled_surface = pygame.transform.scale(
                screen, (int(SCREEN_WIDTH * 0.75), int(SCREEN_HEIGHT * 0.75))
            )
        else:  # high quality
    ...
    def generate_replay_gif(self, stats, player_model="Default", challenge_mode=None):
        ...
        Thread(
            target=self._create_gif_async,
            args=(filepath, stats, player_model, challenge_mode),
        ).start()
    ...
    def _add_stats_overlay(
        self, image, stats, player_model, challenge_mode, frame_number, total_frames
    ):
        ...
        draw.text(
            (margin + 10, y_offset),
            f"Dodged: {stats['obstacles_dodged']}",
            fill=text_color,
            font=font,
        )
        y_offset += 20
        ...
        draw.text(
            (margin + 10, y_offset),
            f"Powerups: {stats['powerups_collected']}",
            fill=text_color,
            font=font,
        )
        y_offset += 20
        ...
        draw.text(
            (image.width - watermark_width - 10, image.height - 25),
            watermark_text,
            fill=(128, 128, 128),
            font=small_font,
        )
        ...
        # Background bar
        draw.rectangle(
            (10, bar_y, 10 + bar_width, bar_y + bar_height), fill=(64, 64, 64)
        )
        # Progress bar
        draw.rectangle(
            (10, bar_y, 10 + int(bar_width * progress), bar_y + bar_height),
            fill=(0, 255, 128),
        )
