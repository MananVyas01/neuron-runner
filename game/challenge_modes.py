...
class ChallengeMode:
    def __init__(
        self, name, display_name, description, duration, effects, visual_changes
    ):
        self.name = name
...
class ChallengeManager:
    ...
    def update(self, game_score, game_time):
        ...
        if (
            game_time - self.last_challenge_time > self.challenge_interval
            and not self.active_challenge
            and game_score > 1000
        ):
            if self.trigger_random_challenge(game_score):
                self.last_challenge_time = game_time

    def apply_challenge_effects(
        self, obstacle_manager, powerup_manager, background_manager
    ):
        """Apply effects of active challenge to game systems"""
        if self.active_challenge:
            self.active_challenge.apply_effects(
                obstacle_manager, powerup_manager, background_manager
            )
    ...
    def draw_challenge_notification(self, screen):
        ...
        # Border
        pygame.draw.rect(
            screen, COLORS["WHITE"], (notif_x, notif_y, notif_width, notif_height), 3
        )

        # Challenge icon and title
        title_text = self.font_large.render(
            f"🚨 {self.active_challenge.display_name.upper()}", True, COLORS["WHITE"]
        )
        ...
        duration_text = self.font_medium.render(
            f"Duration: {remaining_time}s remaining", True, COLORS["WHITE"]
        )
    ...
    def draw_challenge_hud(self, screen):
        ...
        title_text = self.font_small.render(
            f"ACTIVE: {self.active_challenge.display_name}", True, COLORS["WHITE"]
        )
        ...
        # Progress fill
        progress_ratio = self.active_challenge.timer / (
            self.active_challenge.duration * FPS
        )
        fill_width = int(progress_width * progress_ratio)
        pygame.draw.rect(
            screen, border_color, (progress_x, progress_y, fill_width, progress_height)
        )

        # Time remaining
        remaining_time = self.active_challenge.timer // FPS
        time_text = self.font_small.render(
            f"Time: {remaining_time}s", True, COLORS["WHITE"]
        )
        screen.blit(time_text, (panel_x + 10, panel_y + 50))

        # Intensity indicator
        intensity_text = self.font_small.render(
            f"Intensity: {self.active_challenge.intensity:.1f}x", True, COLORS["WHITE"]
        )
