...
class LossGraph:
    ...
    def draw(self, screen, loss_history):
        ...
        pygame.draw.line(self.surface, COLORS['WHITE'], 
                        ...
                        (self.rect.width - 10, self.rect.height - 10), 2)  # X-axis
        pygame.draw.line(self.surface, COLORS['WHITE'], 
                        (10, 10), 
                        (10, self.rect.height - 10), 2)  # Y-axis
...
class UI:
    ...
    def draw_hud(self, screen, stats, player_status):
        ...
        score_text = self.font.render(f"Training Progress: {stats.score:,}", True, COLORS['WHITE'])
        epoch_text = self.font.render(f"Epoch: {stats.current_epoch}", True, COLORS['ACTIVATION'])
        loss_text = self.font.render(f"Loss: {stats.current_loss:.4f}", True, COLORS['GRADIENT'])
        accuracy_text = self.font.render(f"Accuracy: {stats.training_accuracy:.1f}%", True, COLORS['ACTIVATION'])
        ...
        layers_text = self.small_font.render(f"Network Layers: {player_status['layers']}", True, COLORS['NEURON_BLUE'])
        speed_text = self.small_font.render(f"Optimizer Speed: {player_status['optimizer_speed']:.1f}x", True, COLORS['OPTIMIZER'])
        lr_text = self.small_font.render(f"Learning Rate: {player_status['learning_rate']:.3f}", True, COLORS['WHITE'])
        ...
        if player_status['boosted']:
            boost_text = self.small_font.render("🔥 LAYER BOOST - ENHANCED PERFORMANCE", True, COLORS['ACTIVATION'])
        ...
        if player_status['immune']:
            immune_text = self.small_font.render("🛡️ IMMUNITY ACTIVE - DAMAGE BLOCKED", True, COLORS['DATASET'])
        ...
        if player_status['frozen']:
            frozen_text = self.small_font.render("❄️ VANISHING GRADIENT - CONTROLS FROZEN", True, COLORS['LOSS'])
        ...
        graph_title = self.small_font.render("Training Loss Curve", True, COLORS['WHITE'])
    ...
    def draw_excitement_indicators(self, screen, player_status):
        ...
        if player_status['combo'] > 0:
            combo_text = self.font.render(f"🔥 COMBO x{player_status['combo']}", True, COLORS['DATASET'])
            ...
            screen.blit(combo_text, (SCREEN_WIDTH - combo_text.get_width() - 20, y_start + 5))
            y_start += 40
        
        # Adrenaline mode indicator
        if player_status['adrenaline']:
            adrenaline_text = self.font.render("⚡ ADRENALINE MODE!", True, COLORS['LOSS'])
            adrenaline_bg = pygame.Surface((adrenaline_text.get_width() + 20, 30))
            adrenaline_bg.set_alpha(150)
            adrenaline_bg.fill(COLORS['LOSS'])
            screen.blit(adrenaline_bg, (SCREEN_WIDTH - adrenaline_text.get_width() - 30, y_start))
            screen.blit(adrenaline_text, (SCREEN_WIDTH - adrenaline_text.get_width() - 20, y_start + 5))
            y_start += 40
        
        # Close call indicator
        if player_status['close_call']:
            close_call_text = self.font.render("💫 CLOSE CALL!", True, COLORS['ACTIVATION'])
            ...
            screen.blit(close_call_bg, (SCREEN_WIDTH - close_call_text.get_width() - 30, y_start))
            screen.blit(close_call_text, (SCREEN_WIDTH - close_call_text.get_width() - 20, y_start + 5))
    ...
    def draw_health_bar(self, screen, activation_level, x, y):
        ...
        if fill_width > 0:
            pygame.draw.rect(screen, health_color, (x + 2, y + 2, fill_width, bar_height - 4))
        
        # Health text
        health_text = self.small_font.render(f"Neural Activation: {activation_level:.0f}%", True, COLORS['WHITE'])
    ...
    def draw_performance_indicators(self, screen, stats, player_status):
        ...
        # Title
        title_text = self.small_font.render("Performance Metrics", True, COLORS['WHITE'])
    ...
    def draw_game_over(self, screen, stats):
        ...
        # Game over text
        game_over_text = pygame.font.Font(None, 72).render("TRAINING FAILED", True, COLORS['LOSS'])
        game_over_rect = game_over_text.get_rect(center=(SCREEN_WIDTH//2, SCREEN_HEIGHT//2 - 100))
        screen.blit(game_over_text, game_over_rect)
        
        # Final stats
        final_score_text = self.font.render(f"Final Training Progress: {stats.score}", True, COLORS['WHITE'])
        final_score_rect = final_score_text.get_rect(center=(SCREEN_WIDTH//2, SCREEN_HEIGHT//2 - 50))
        screen.blit(final_score_text, final_score_rect)
        
        epochs_text = self.font.render(f"Epochs Completed: {stats.current_epoch}", True, COLORS['WHITE'])
        epochs_rect = epochs_text.get_rect(center=(SCREEN_WIDTH//2, SCREEN_HEIGHT//2 - 20))
        screen.blit(epochs_text, epochs_rect)
        
        high_score_text = self.font.render(f"Best Training Progress: {stats.high_score}", True, COLORS['DATASET'])
        high_score_rect = high_score_text.get_rect(center=(SCREEN_WIDTH//2, SCREEN_HEIGHT//2 + 10))
        screen.blit(high_score_text, high_score_rect)
        
        # Restart instruction
        restart_text = self.font.render("Press R to Restart Training", True, COLORS['ACTIVATION'])
...
def draw_background(screen, scroll_offset=0, player_status=None):
    ...
    if player_status and player_status.get('adrenaline', False):
        ...
        adrenaline_bg = (COLORS['BACKGROUND'][0] + red_tint, COLORS['BACKGROUND'][1], COLORS['BACKGROUND'][2])
    ...
    grid_size = 50
    grid_alpha = 60 if not (player_status and player_status.get('adrenaline', False)) else 120
    
    for x in range(-grid_size, SCREEN_WIDTH + grid_size, grid_size):
        adjusted_x = (x - scroll_offset) % SCREEN_WIDTH
        grid_color = (30, 40, 60) if not (player_status and player_status.get('adrenaline', False)) else (60, 30, 30)
        pygame.draw.line(screen, grid_color, 
                        (adjusted_x, 0), (adjusted_x, SCREEN_HEIGHT), 1)
