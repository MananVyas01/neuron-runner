...
class GameState:
    ...
    def __init__(
        self, player_pos, obstacles, powerups, speed, score, additional_context=None
    ):
        ...
        # Additional context
        self.time_since_last_powerup = (
            additional_context.get("time_since_last_powerup", 0)
            if additional_context
            else 0
        )
        self.combo_multiplier = (
            additional_context.get("combo_multiplier", 1.0)
            if additional_context
            else 1.0
        )
        self.difficulty_level = (
            additional_context.get("difficulty_level", 1.0)
            if additional_context
            else 1.0
        )
...
class AdvancedQLearningAgent:
    ...
    def __init__(
        self, state_size=75, action_size=6, learning_rate=0.001, architecture="dqn"
    ):
        self.state_size = state_size
        self.action_size = (
            action_size  # 0=nothing, 1=jump, 2=duck, 3=boost, 4=special, 5=combo
        )
        ...
        print(
            f"🤖 Advanced Q-Learning Agent initialized with {architecture} architecture"
        )
    ...
    def get_q_values(self, state, network=None):
        ...
        if network["type"] == "linear":
            ...
            h1 = np.maximum(
                0, np.dot(state, network["weights1"]) + network["bias1"]
            )  # ReLU
            h2 = np.maximum(
                0, np.dot(h1, network["weights2"]) + network["bias2"]
            )  # ReLU
    ...
    def act(self, state, training=True):
        ...
        if training and np.random.random() <= self.epsilon:
            ...
            if np.random.random() < 0.3:  # 30% truly random
                ...
                weights = np.array(
                    [
                        np.mean(self.action_success_rates.get(i, [0.5]))
    ...
    def replay(self):
        ...
        priority_batch = random.sample(
            list(self.priority_memory), min(priority_size, len(self.priority_memory))
        )
        ...
        if self.double_dqn:
            ...
            next_actions = np.argmax(
                [self.get_q_values(s) for s in next_states], axis=1
            )
            next_q_values = np.array(
                [
                    self.get_q_values(s, self.target_network)[a]
                    for s, a in zip(next_states, next_actions)
                ]
            )
        else:
            # Standard DQN
            next_q_values = np.max(
                [self.get_q_values(s, self.target_network) for s in next_states], axis=1
            )
    ...
    def save_model(self, filepath):
        ...
        model_data = {
            ...
            "action_success_rates": {
                str(k): list(v) for k, v in self.action_success_rates.items()
            },
            "meta_info": {
        ...
        try:
            ...
            if "action_success_rates" in model_data:
                self.action_success_rates = {
                    int(k): deque(v, maxlen=50)
                    for k, v in model_data["action_success_rates"].items()
                }
...
class AICompetition:
    ...
    def create_new_bot(self, name, architecture="dqn", hyperparams=None):
        ...
        self.bots[name] = AdvancedQLearningAgent(
            architecture=architecture, **(hyperparams or {})
        )
    ...
    def stop_training(self, bot_name):
        ...
        print(
            f"   📊 Performance: {self.bots[bot_name].stats.average_score:.1f} avg score"
        )
    ...
    def save_bot_analytics(self, bot_name):
        ...
        analytics = {
            ...
            "action_preferences": {
                str(k): np.mean(v) for k, v in bot.action_success_rates.items()
            },
    ...
    def update_bot_training(self, old_state, action, reward, new_state, done):
        ...
        self.current_bot.remember(
            old_vector, action, reward, new_vector, done, priority
        )
    ...
    def get_comprehensive_bot_stats(self, bot_name):
        ...
        recent_performance = (
            list(bot.training_history["episode_rewards"])[-20:]
            if bot.training_history["episode_rewards"]
            else [0]
        )
        performance_trend = (
        ...
        return {
            ...
            "action_analysis": {
                ...
                "most_successful_action": (
                    max(bot.action_success_rates.items(), key=lambda x: np.mean(x[1]))[
                        0
                    ]
                    if bot.action_success_rates
                    else 0
                ),
            },
        }
    ...
    def calculate_learning_efficiency(self, bot):
        ...
        recent_performance = np.mean(
            list(bot.training_history["episode_rewards"])[-10:]
        )
    ...
    def create_bot_tournament(
        self, tournament_name, bot_names, tournament_type="round_robin"
    ):
    ...
    def human_vs_ai_challenge(self, bot_name, challenge_type="score_battle"):
        ...
        print(
            f"   🤖 Bot Level: {bot_stats['basic_stats']['average_score']:.0f} avg score"
        )
        print(f"   🏆 Bot Best: {bot_stats['basic_stats']['best_score']}")
        print(
            f"   📊 Bot Consistency: {bot_stats['performance_analysis']['consistency_score']:.1f}%"
        )
...
class AIGameInterface:
    ...
    def get_current_state(
        self, player, obstacle_manager, powerup_manager, stats, additional_context=None
    ):
        ...
        enhanced_context = {
            ...
            "recent_performance": (
                np.mean([s.get("score", 0) for s in list(self.state_history)[-10:]])
...
class AdvancedRewardCalculator:
    ...
    def calculate_reward(
        self, stats, player, collision_occurred, powerup_collected, near_miss=False
    ):
        ...
        obstacles_dodged_increase = (
            getattr(stats, "obstacles_dodged", 0) - self.last_obstacles_dodged
        )
        ...
        powerups_collected_increase = (
            getattr(stats, "powerups_collected", 0) - self.last_powerups_collected
        )
        ...
        if stats.score > self.performance_baseline:
            consistency_bonus = min(
                (stats.score - self.performance_baseline) * 0.01, 10
            )
        ...
        self.performance_baseline = (
            self.performance_baseline * 0.99 + stats.score * 0.01
        )
...
try:
    ...
    if not AI_AVAILABLE:
        print(
            "⚠️ AI Competition features disabled - install numpy for full functionality"
        )
except Exception as e:
