...
class FactCardSystem:
    ...
    def show_random_card(self):
        ...
        for fact in available_cards:
            time_weight = max(1, (current_time - fact.last_shown) / 3600)  # Hours since last shown
            frequency_weight = max(1, 5 - fact.shown_count)  # Less shown = higher weight
        ...
        print(f"📚 Showing fact card: {self.current_card.category} - {self.current_card.question[:50]}...")
    ...
    def draw(self, screen):
        ...
        card_rect = pygame.Rect(self.card_x, current_y, self.card_width, self.card_height)
        ...
        pygame.draw.rect(screen, category_color, category_rect, border_top_left_radius=15, border_top_right_radius=15)
        
        # Category text
        category_font = pygame.font.Font(None, 32)
        category_text = category_font.render(f"📚 {self.current_card.category}", True, COLORS['WHITE'])
        ...
        question_lines = self.wrap_text(self.current_card.question, question_font, card_rect.width - 40)
        ...
        answer_lines = self.wrap_text(self.current_card.answer, answer_font, card_rect.width - 40)
        ...
        if self.current_card.fun_fact:
            ...
            fun_fact_lines = self.wrap_text(f"💡 {self.current_card.fun_fact}", fun_fact_font, card_rect.width - 40)
        ...
        if self.auto_advance:
            ...
            progress_rect = pygame.Rect(card_rect.x + 20, card_rect.bottom - 30, card_rect.width - 40, 6)
            pygame.draw.rect(screen, COLORS['LIGHT_GRAY'], progress_rect, border_radius=3)
        ...
        instruction_text = instruction_font.render("Press SPACE, ENTER, or click to continue", True, COLORS['GRAY'])
        instruction_rect = instruction_text.get_rect(center=(card_rect.centerx, card_rect.bottom - 10))
    ...
    def get_learning_stats(self):
        ...
        return {
            ...
            'completion_percentage': (len(self.categories_learned) / len(set(f.category for f in self.facts))) * 100 if self.facts else 0
        }
